...
async def _remember_target(
    ...
    不带 category 就不记，这也是"记不记"的唯一开关：搜索页的保存位置弹窗只在
    用户勾了「记住本次选择」时才带上分类（见 download-target-dialog.tsx），
    其他调用方——订阅投递、CLI——本来就不该产生用户级记忆，同样不带。
    "选了库但没开智能入库"这种组合三种 kind 都表达不了，也跳过：记不准不如不记。
