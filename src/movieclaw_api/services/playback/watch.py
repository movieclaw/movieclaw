...
def web_client_info(*, device_id: str, user_agent: str | None) -> ClientInfo:
    ...
    """
    ua = user_agent or ""
    app = _APP_USER_AGENT.search(ua)
    if app is not None:
        client_name, fallback_device = _APP_CLIENTS[app["platform"]]
        ...
        if system is not None:
            ...
            os_label = f"{system['os']} {system['version']}"
            device = os_label if device == system["os"] else f"{device} · {os_label}"
        return ClientInfo(
            name=client_name,
            device_name=device,
            device_id=device_id,
