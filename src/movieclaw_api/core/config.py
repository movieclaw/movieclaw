...
class Settings(BaseSettings):
    ...
    # 每个数据库连接的 SQLite 页缓存上限（MB）。空间换 IO：库一大，海报墙、
    # 筛选栏这类聚合查询会把同一批页反复读回来，SQLite 自带的 2MB 默认值
    # （约 500 页）根本兜不住。实测 158MB 的库（3.7 万条目 / 7 万文件）：
    # 筛选栏一次请求的读系统调用 122443 → 1854，海报墙 13838 → 797。
    # **按需增长、不预分配**：小库摸不到那么多页，内存开销实测为零；
    # 大库在 8 路并发下常驻内存也只多约 40MB。上限 = 连接池上限 × 本值，
    # 内存吃紧的设备可调小（设 0 则不干预，用 SQLite 默认）。
    db_cache_mb: int = Field(default=32, ge=0, alias="MOVIECLAW_DB_CACHE_MB")
