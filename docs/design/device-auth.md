...
```
┌───────────────────────────────────────────────┐
│  [icon]  MovieClaw Transcoder                 │
│          把这台 Mac 的硬件编码器借给 movieclaw  │
│                                               │
│  ┌─────────────────────────────────────────┐  │
│  │ ● 尚未连接                               │  │
│  │ 填好上面的地址，点「连接并配对」。         │  │
...
│  服务器                                        │
│  ┌─────────────────────────────────────────┐  │
│  │ 地址   [http://10.1.1.5:3000          ] │  │
│  │        [在局域网中查找]                  │  │
│  └─────────────────────────────────────────┘  │
│  请填局域网地址和端口。转码要来回传输大量视频     │
...
- 视觉上按系统设置的语言组织：抬头（图标 + 名字 + 一句话）、状态卡、
  分组小标题 + 圆角卡片、卡片下灰色脚注；窗口高度随内容自适应，展开
  「高级设置」不会把按钮栏挤出可视区。实现拆在 `SettingsViews.swift`
  （纯外观）与 `SettingsWindowController.swift`（状态机）两个文件里。
...
未授权 ──连接并配对──> 连接中 ──失败──> 失败（错误就地显示，可重试）
                        ...
                        ↓
                     等待批准（显示 user_code + 轮询）
                        │批准            │拒绝/超时/取消
                        ↓                ↓
                     已授权 ←────────  失败 / 回到未授权
```

「已授权」是稳态：之后开机自动连接，用户不需要再打开这个窗口。回到这个窗口
时主按钮变成「保存并测试连接」——改地址、改端口不该动令牌；换机器或被吊销后
走旁边的「重新配对…」，它会先弹一次确认再替换本机令牌。
