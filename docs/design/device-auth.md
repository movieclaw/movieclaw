...
**v1 的批准者只有超管**（成员能否批准见 §4.4）。所以现阶段的实际形态是：
...
| 令牌泄漏 | 每设备独立、可单独吊销、可归因；Worker 令牌被业务接口默认拒绝。CLI 令牌是全权的（§4.4），止损手段是设备列表里的一键吊销 |
...
- **成员不能批准设备**。设备页整体只对超管开放。代价是 v1 签不出受限令牌
  （§4.4 已写明后果与现有的三道操作纪律）。模型本身留好了口子，将来打开
  只需两处加法，不需要推翻。
...
1. **超管改密时是否清空全部设备令牌**。改密只轮换签名密钥，Cookie 会话与
   Agent 令牌瞬间失效，但落库的设备令牌不受影响。v1 的令牌全是超管全权，
   这个缺口比有成员分流时更值得关注。倾向：改密时给一个明确的选项
   「同时吊销所有已连接的设备」，默认不勾——改密的动机多半是密码本身泄漏
   而非设备失窃，静默踢掉所有 Worker 的观感很差。
2. **令牌是否默认设过期**。默认长期有效最省事，但自部署用户的令牌泄漏是
