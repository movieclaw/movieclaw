...
**2026-09 修订：引导 + 分页。** 上面的原则不变（唯一必填是地址、配对一次点击、
等批准时只剩配对码、配好后地址只读、稳态唯一出口是「断开并重新配置」），形态改为：

- **没配对时是三步引导**（`OnboardingViewController.swift`）：连接 → 批准 → 完成。
  「连接」一步把局域网查找结果直接列成可点的卡片，手填地址退为备选；「完成」一步
  同时显示 Jellyfin-ffmpeg 的下载进度（原来单独弹出的下载窗已并入设置）。
- **配好之后是 toolbar 样式的三页**（`SettingsPanes.swift`）：连接（状态、名称、
  自动连接、断开并重新配置）/ 转码（ffmpeg 来源与能力检测、硬件编码器、并发）/
  关于。改动即时生效、没有保存按钮；只有改名、换 ffmpeg、调并发才重启连接，手上
  有任务时先确认。原「高级设置」四项因此不再折叠。
- 分组底色改为前景色薄罩：macOS 27 上 `controlBackgroundColor` 与窗口底色同值，
  加上液态玻璃下去掉了描边，原来的分组会整块隐形。
