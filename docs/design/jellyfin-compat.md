...
1. **显式配置优先**：设置 `published_server_url`（`jellyfin.compat` 配置域，
   等价 Jellyfin 的 "Published Server URL"）。配置了就原样返回。它是高级项，
   **没有界面入口**——用户可见的入口是第 3 步的「外部访问地址」。
2. **自动探测兜底**（未配置时）：对发现报文的来源地址做一次 UDP
   `connect()`（不发包），读 socket 本地地址——内核路由表选出的、面向该
   客户端的出口 IP。host 网络模式或裸机部署下，这就是正确的局域网 IP；
   端口用应用实际监听端口。
3. **探测结果自检**：探测出的 IP 落在 Docker 网段（172.16.0.0/12）时，改用
   「设置 → 网络 → 外部访问地址」（`app.server.external_url`）应答——容器内
   看到的 IP 和端口都可能与宿主映射不一致，任何容器内探测都无法得知宿主的
   真实映射，用户填的地址是桥接部署下唯一可靠的答案。外部访问地址只在这一步
   兜底、不前置：它可能是公网域名，host 网络下探测出的局域网 IP 更直接。
   外部访问地址也没配时**仍然应答**探测 IP（有意偏离 Jellyfin 的"取不到
...
- **兼容层默认开启**（用户决策 2026-08-03：接口本身成本不高，开箱即用价值
  ...
  播放器连接；控制台提供关闭开关，Docker 桥接下的自动发现地址取「设置 →
  网络 → 外部访问地址」（§3.1）。
