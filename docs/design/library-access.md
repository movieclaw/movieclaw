...
| 按时间 / 按库 / 全部 | 媒体库首页「最近观看」标题右侧 ⋯ → 今天 / 最近一周 / 全部 / 某个媒体库（弹窗下拉选库） | `since` 时间窗口按浏览器本地时间算起点（今天 = 当地 0 点）传给接口；按库清该库内全部条目 |
| 清空全部记录 | 设置 → 个人信息 → 「观看历史」卡片 → 「清空全部观看记录」 | 全部 |

- 单库页 ⋯ 菜单**不再**放清空入口：观看记录是跨库的个人数据，入口长在展示
  它的「最近观看」上；每进一个库都看到一条破坏性操作既多余也不在正确的位置。
...
| `library-detail-view.tsx` | `viewer_access=false` 时的管理视图 |
| `recent-watch-menu.tsx` | 首页「最近观看」⋯ 菜单：按今天 / 最近一周 / 全部 / 某个库清空 |
...
2. **活动页的范围外记录用计数字段而不是占位行**：`MediaActivityView` 加
   ...
   activity.md「范围切换」。活动页撤掉「最近观看」后该字段随之删除，同一
   口径由播放记录的 `hidden_count` 承担。）
