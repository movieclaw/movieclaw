...
复选框「记住本次选择」**删除**，且不以任何形式回归（理由见 §6）。记忆改为每次
提交后自动写入（仅当目标与已存记忆不同时才写，避免批量下载时的无谓写入）。
...
| 操作 | 落点 |
|---|---|
| **建立**记忆 | 完整弹窗提交后自动写入，无需任何勾选 |
| **更新**记忆 | 确认条「更改」→ 弹窗改选 → 提交即覆盖该分类 |
| **清除**记忆 | 确认条底部的「不再记住」——清掉该分类并展开完整弹窗 |

一个操作一个入口，没有复选框。**特别是不要再引入任何形式的「记住本次选择」
勾选框**：原实现的失败正是因为它默认不勾、且要求用户预判「以后还会不会下同类」。
现在改为提交即记住，用户永远不需要预判未来，只需要在看到错误默认时把它清掉。

完整弹窗底部保留一行**非交互**说明（不是勾选框），告知这次选择会成为该分类的
默认位置、之后可随时更改，让记忆的建立对用户可见而不需要他做决定。
