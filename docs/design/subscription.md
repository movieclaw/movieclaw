...
- 每 5 分钟读取下载器的 `completed_bytes`。暂停、排队、校验不计时；全部下载器
  不可达只表示状态未知；可达但连续三次查无任务才确认缺失。用户在活动页亲手删除
  主源时"消失"已是确定事实，删除接口当场把工单退回 wanted，不再等三次确认；
  已完成但落点核验失败（movieclaw 看不到文件）的任务同样适用，红灯随之熄灭。
- 下载器报告任务本身出错（qBittorrent 的 `missingFiles`/`error`、Transmission 的
  本地错误）**不是死种**：换源只会把新源投进同一个出故障的下载器，试用超时→再搜
  →再投循环。此时冻结 15/30 分钟计时、不发起待办搜索，时间线记一条原因，等用户
  在下载器里处理后从头计时。Transmission 的 tracker 错误不归入此类——种子从站点
  撤下正是没有做种的死种，按 stalled 交给换源。
- 投递目录处在**目录级落点故障**中（movieclaw 看不见该目录：挂载失效/映射缺失，
  见 activity.md「红灯的粒度等于修复动作的粒度」）时，自动换源刹车、一小时后再看：
  目录都看不见，新源同样"无法入库"、30 分钟后试用失败、再换下一个，真实事故里
  45 小时白烧 90GB。手动「立即换种」不受限；用户忽略红灯即松开刹车。同订阅、
  同单元、同原因的停滞事件 24 小时内折叠为一条计数，不让流水淹没真信号。
