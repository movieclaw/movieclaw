...
**补录模型表单**（设置 → AI 模型，回答「自定义模型怎么配档位」）：

现有表单已有「支持思考」开关与「思考预算」输入，扩展为：
...
取舍：Maka 的**批量声明行不做**——movieclaw 的补录是逐模型表单而非
多模型能力表格，没有 模型数×档位数 的点击爆炸问题；将来若把补录改成
表格式再补。同样照抄的原则：**内置预设模型不给声明控件**（目录已声明，
UI 不承诺存不下来的编辑）；已存声明里出现的词汇表外值保持可见可取消。
