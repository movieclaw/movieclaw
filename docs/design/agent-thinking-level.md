...
**会话 composer**：档位并进模型 pill 的下拉里——强度是「当前模型的强度」，
是模型的从属设置，不与模型平起平坐占两个 pill（2026-09 合并，此前是加号旁
两个并排 pill，小屏被挤成两截省略号，且孤零零一个「高」看不出是谁的高）。

```
┌──────────────────────────┐
│ 模型                     │  超一屏内部滚动，打开时把当前项滚进视野
│ ✓ Claude Opus 5          │
│   GPT-5.2（备用实例）     │
├──────────────────────────┤
│ 思维链强度  高   恢复默认 │  下半按当前模型的菜单渲染
│ 更快 ●──●──◉──● 更聪明    │
└──────────────────────────┘
   [ Claude Opus 5 ⌄ ]        pill 只写模型名，不带档位后缀
```

下半的形态由该模型的菜单长度决定（`lib/thinking-level-control.ts`）：两档以上
是离散滑杆，只有一档（toggle 方言的单项「关」）是两格分段「开启 / 关闭」，
空菜单则整段不渲染、留一行灰字说明该模型不支持调节。选模型即收起菜单、并
清空已选档位（旧档位在新模型的菜单里未必存在）；调档位不收起。选中值随消息
提交，写入转录信封。
