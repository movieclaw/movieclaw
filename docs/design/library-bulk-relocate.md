...
```
转移计划：593 部选中，586 部可搬（7 部同名冲突将跳过）；跨盘复制 29.2 TiB，
目标盘剩余 36.4 TiB（需要 30.7 TiB，够）；其中 210 部与做种目录有硬链接，
复制后做种链接断开、源盘不会释放这 10.1 TiB。
```
...
5. **批量转移要不要支持「目标同名时合并进去」**？当前一律跳过（继承单条目的
   「绝不覆盖、绝不合并」）。合并库场景下，两个库都有同一部片的不同版本是常见的，
   用户可能就是想让它们并到一个目录里。这是本设计里唯一一处我认为可能需要
   放宽、但不敢自己放宽的地方。
