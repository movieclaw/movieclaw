...
- **每行就是一条，没有展开态**：把手 · 类型图标 · 名字（库行直接在行上改，占位文字
  就是推荐名，留空即跟随排序）+ 小字「来源 · 排序」· **排序下拉框** · 「未看」小开关
  （仅库行）· 眼睛 · 删除（自加行）。
...
- 「＋ 添加一行」只问一个问题：**从哪来**。选一个库得到「最近添加的 X」，选一个
  合集得到它本身；已在首页的合集置灰标「已在首页」；内置的「我的收藏」合集不进候选。
...
同一个页面，窄一点：控件区（排序下拉、未看、眼睛、删除）整体换到第二行靠右，名字与
小字独占第一行不被挤断；把手常驻在左；排序下拉是原生 `<select>`，手机上弹系统选单。
...
- 与初稿的差异：合集行不区分规则 / 手动，同一组预设；自定义页不放海报；行不展开，排序
  是下拉框，名字直接在行上改；去掉上下箭头只留把手拖拽；页头两个入口都是图标钮
