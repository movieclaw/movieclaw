...
- **控制行**（贴底）：左端时间，右簇**只有三个键：字幕 / 设置 / 横屏**。
  一条控制条上每多一个图标，用户就要多认一次；能塞进菜单的就不占位置。
  - 片名不放这里——顶栏已经有了。
  - 音量条没有：网页播放器上调音量的人远比想象中少（系统音量、耳机、键盘
    `M` 与上下键都能管），留着只是让静止画面多一件东西。
  - 「下一集」没有：切集有片尾倒计时卡片与媒体键 `nexttrack` 两条更自然的
    路，控制条上再放一个是第三条。
