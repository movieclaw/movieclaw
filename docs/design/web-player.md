...
**线路装不下码率不走逐级降**（2026-09-13 补）：直出 / remux / 音频单转的视频
都是 `-c:v copy`，码率改不了。线路不够时逐级降到 remux、再到音频单转，码率一分
没少，照样缺粮，连败两次才一步到软转把码率压下来——用户看到的是一分多钟转圈
外加三次黑屏重开。所以缺粮判定叠加「实测线路 < 源码率」（`bandwidth.ts` 的
`bandwidthDegradeWanted`）时，把三个直通档一并标掉（`machine.ts` 的
`bandwidth-degrade`），直接带实测带宽去开转码会话，服务端 `adapt_to_downlink`
按线路压码率、必要时降高度。这条不计入连败：它不是「这一档播不了」的证据。
档 0 的缺粮上限同时从 45 秒收到 15 秒（`stall.ts` 的 `DIRECT_STARVE_TIMEOUT_S`）：
45 秒是给转码器追上来留的，直出没有转码器可等。
...
- **设置页总开关**（2026-09-09）。`playback.policy.trickplay_enabled` 默认开，
  关掉后不再生成新预览（已生成的照常可读，重新打开即恢复）；全部触发点都经
  `schedule()`，一处拦截全覆盖。
