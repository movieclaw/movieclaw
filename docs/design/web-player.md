...
**进度条常驻播放器最底边**（收起后）。控制条展开时它在时间与操作区之间；
收起时下方那一整块塌成 0 高，它自然落到最底边，只剩一条贴边的细线
（`h-5 → h-[3px]`、`px-6 → px-0`）。「安静时也知道播到哪」与「安静时画面
干净」只有这样才能同时成立。

**三段同时可见，对比度靠进度条自带的暗渐变而不是靠染黑轨道**。收起后进度条
直接压在原始画面上，那里连渐变都没有，而三段（未播 / 已缓冲 / 已播）全是浅色，
背后是雪景还是黑夜完全不可控。这里试过两版：
...
- **现在这版：轨道回到半透白，另给进度条铺一条随身的暗渐变**（`.player-scrub-shade`
  ...
  只是我们的进度条收起后仍常驻，所以 scrim 也跟着缩成常驻的一薄条。
