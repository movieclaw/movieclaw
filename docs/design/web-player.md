...
**轨道必须是暗色**。收起后进度条直接压在原始画面上，那里连渐变都没有，而
已播段是浅银——遇到雪景 / 白墙 / 爆炸，浅银配常见的半透白轨道会一起消失
（实测在近白画面上整条几乎看不见）。改成 `rgb(0 0 0 / .6)` 的暗轨道，整条
变成「暗底 + 亮填充」，亮暗两种画面上都读得出来；再加一道向下的暗投影把它
从亮画面上抬起来。代价是暗画面里轨道会融进背景、看不出整片有多长，但缓冲段
仍然给得出参照。**这是把强调色从 Netflix 红换成产品冷银之后必然要还的债**：
红色是中深色、在黑白两种底上都成立，浅银不是。
...
| ⑳ | **浅色进度条 + 半透白轨道** | 亮画面（雪景/白墙）上控件收起后整条进度条消失 | 轨道改暗色 + 暗投影；控件本身装进磨砂卡片（§6.8） |
