...
- [~] JASSUB / VTT 两条字幕路径（外挂与内封轨均可播）+ 时间轴微调 + 样式配置；
      **libbitsub（PGS）与内嵌字体抽取未做**
- [x] 自动降档回路（§6.3）
...
- **MKV 内嵌字体抽取、PGS 前端渲染（libbitsub）**。内封的**文本与 ASS 轨已经
  可播**（服务端按需 ffmpeg 抽取，见 §12.8）；缺的是 ASS 依赖的内嵌字体——
  没有它，番剧字幕会回退成默认字体。PGS 是位图轨，仍进「不可用」清单并给中文
  原因，不是给一个点了没反应的选项。
