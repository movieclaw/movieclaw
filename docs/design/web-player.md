...
- **时间**：进度条**上方**、左对齐的一颗磨砂药丸。
- **进度条**：见下。
- **操作区**：进度条**下方**。左边是一张磨砂卡片，装本片的三个操作（字幕 /
  设置 / 横屏）；右边是切集胶囊。左右分工照 YouTube 全屏：左边是「对这一路
  播放的操作」，右边是「去下一个内容」，两类东西不混在一排里。
  - 全部装在磨砂玻璃里（`.player-glass`，语言对齐站内 `.notice-surface`）：
    画面亮度完全不可控（雪景、白墙、爆炸），只靠渐变压不住，按钮会时不时糊
    进画面。
  - 一条控制条上每多一个图标，用户就要多认一次，所以只留三个：片名不放这里
