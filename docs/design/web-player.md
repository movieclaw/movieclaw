...
- 画中画：已按「只做自动、不给按钮」实现（§6.8）；Document PiP 待评估。
...
**画中画没有按钮，只做自动**。「切走标签页就自动进画中画」网页自己实现不了：
...
「横屏」按钮正好就是全屏——**前提是播放器在页面隐藏时不能暂停**。既然两条
路都不需要用户点，控制条上就不留这个键：Firefox 上它本来还是个点了没反应的
死按钮（那边的画中画只活在浏览器自己的界面里，网页调不动）。
