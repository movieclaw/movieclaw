...
- **控制行**（贴底，进度条之上）：**左下角**是时间 + 三个键（字幕 / 设置 /
  横屏），**右下角**是切集入口。左右分工照 YouTube 全屏：左边是「对这一路
  播放的操作」，右边是「去下一个内容」，两类东西不混在一排里。
  - 时间单独占一行、压在功能键上方：和功能键挤一行会让「读数」和「可点」
    混在一起，用户得先分辨哪个能按。
  - 一条控制条上每多一个图标，用户就要多认一次，所以只留三个：片名不放这里
    （顶栏已经有了）；音量条没有（系统音量、耳机、键盘 `M` 与上下键都能管，
    留着只是让静止画面多一件东西）；「播放诊断」收进设置齿轮的菜单里——它是
    排障用的，不是日常操作。
...
- **进度条**：常驻播放器**最底边**。控制条淡出后它不淡出，只是把左右内边距
  和高度收掉，变成一条贴着下沿的细线（`h-5 → h-[3px]`、`px-6 → px-0`）。
