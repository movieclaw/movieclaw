...
2. **关键帧采样**（决策同步路径，三段共约 90 秒码流的读取，慢存储上
   几十秒）：加**条目详情页预热**（`services/playback/warmup.py`）——
   用户看简介的几秒里后台做掉，点播放时缓存直接命中。只预热文件数 ≤ 4
   的条目（整季剧集猜不到要播哪集，全预热是无谓 IO），同条目并发触发去重。
   **只替可能直通的网页客户端采样（2026-09）**：决策接口按「身份 +
   User-Agent」记下客户端上报的能力快照，详情页查到它、且
   `needs_keyframe_probe`（纯判定）认为这部片会走档 1/2 才读盘；App、
   Infuse、脚本与 UI 测试没有网页能力快照，打开详情不再读片子（NAS 实测
   iOS 打开一部 15 GB 的 mkv 详情白读 171 MB）。
   **默认字幕抽取曾经也在预热里，2026-09 移除**：详情接口会被批量调用
   （UI 测试逐个打开几十部电影），每部一次整文件通读，经 NFS 并发平分带宽、
   120 秒全部超时作废，NAS 一天白读几百 GB。现与 Jellyfin 一致按需抽取：
   播放器请求字幕时一趟 ffmpeg 抽出该文件全部内封轨，全局串行、超时按体积
   估算、失败按视频 mtime 记住不重试（`services/media_extract.py`）。
