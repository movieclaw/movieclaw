...
- 季集选择、规则组、目标库沿用现有路由与定格机制；规则组是管理员配置，
  成员订阅一律用自动结论：规则组按适用范围自动选组、都不命中用默认规则组
  （[rule-set-scope.md](rule-set-scope.md)），目标库按收藏范围路由（成员没有
  "选库/选规则"的 UI）。
...
> 2026-09-05 演进：库侧新增可见范围（`access_mode` / `admin_visible`），超管的
> 浏览也走同一条判定，判定不再返回 None。现行规则以
> [library-access.md](library-access.md) 为准，本节保留原始设计与留位论证。
