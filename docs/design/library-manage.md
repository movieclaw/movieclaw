...
│        电影 · 实时监控关 · /remote/media/电影/欧美   334 个文件  128 / 300     │
└───────────────────────────────────────────────────────────────────────────┘
...
| 库（弹性宽） | 缩略图（服务端拼贴图；无海报用类型图标，你不在浏览范围内时是锁）、**库名**（链接到 `/library/{id}`，一行里唯一的白色加粗）、「默认」标、**偏离默认时**的可见范围胶囊（全部成员且自己能看 = 默认，不挂）；第二行小字，顺序是「类型 · 备注 · 根目录」：备注只念**偏离默认的配置**（「从首页排除」、「实时监控关」；不声明收藏范围是正常形态，不提示）；主根目录等宽字体放最后（多根折成 `+N 个根目录`，悬停展开），窄屏下它整体换到下一行而不是把备注拆散；手机端不在这行里，而是在库名块下方独占卡片一整行 | `name` `kind` `is_default` `access_mode` `member_ids` `admin_visible` `viewer_access` `root_paths` `match_rules` `exclude_from_home` `realtime_watch` |
...
| 配置备注 | 「收藏范围 N 项条件」「实时监控开/关」逐行写 | 只写偏离默认的：「从首页排除」「实时监控关」（收藏范围声明与否都不写） |
