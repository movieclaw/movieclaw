...
│ ⠿  │ 库          │ 根目录        │ 库存    │ 状态          │ 可见范围  │ ··· │
└─────┴────────────┴──────────────┴────────┴──────────────┴──────────┴────┘
拖动 ⠿ 调整首页「我的媒体库」的展示顺序，松手即保存      ● 任务进行中 ● 有待处理 ● 有缺失
```
...
|---|---|---|
| 拖拽柄 | ⠿，拖拽改顺序 | — |
| 库 | 4 张最近海报的小缩略图（无海报用类型图标；你不在浏览范围内时是锁）、库名（链接到 `/library/{id}`）、「默认」标；第二行只念**需要留意的配置**：类型 ·（手机端并入可见范围胶囊）· 「收藏范围 N 项条件」或警示色的「未声明收藏范围」· 「从首页排除」。默认态（在首页展示）与「你看不到它」不占字 | `name` `kind` `is_default` `match_rules` `exclude_from_home` `viewer_access` |
...
| 可见范围 | 胶囊：全部成员 / 指定成员 N（只数成员）/ 仅自己 / 无人可见；你本人不在范围内时胶囊前带锁。文案只说库开放给谁，不再退化成「仅管理」——否则超管把自己摘出某库后看不出成员能不能看 | `access_mode` `member_ids` `admin_visible` `viewer_access` |
| 操作 | 唯一一个 ··· 按钮 | — |
...
| 7 | 其余 | 无圆点、无状态词（「空闲」不是需要看的状态，图例里也不列） | — | 只显示 `最近扫描 X 前 · 实时监控开/关` 这行事实 |
...
**手机端**：同一行组件按断点切成卡片（见样稿第三屏）：第一行库名 + 「默认」标 + 可见范围胶囊（与「默认」同一套小标签样式）+ ···，第二行类型与需要留意的配置，
之后是根目录、库存、状态；状态格跨整行，进度条通栏填满卡片（桌面端受列宽约束）。
「创建媒体库」在页头标题右侧。
