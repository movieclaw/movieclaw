...
| 库（弹性宽） | 缩略图（服务端拼贴图；无海报用类型图标，你不在浏览范围内时是锁）、**库名**（链接到 `/library/{id}`，一行里唯一的白色加粗）、「默认」标、**偏离默认时**的可见范围胶囊（全部成员且自己能看 = 默认，不挂）；第二行小字，顺序是「类型 · 备注 · 根目录」：备注只念**偏离默认或有待办的配置**（警示色「未声明收藏范围」、「从首页排除」、「实时监控关」）；主根目录等宽字体放最后（多根折成 `+N 个根目录`，悬停展开），窄屏下它整体换到下一行而不是把备注拆散，手机端固定独占一行 | `name` `kind` `is_default` `access_mode` `member_ids` `admin_visible` `viewer_access` `root_paths` `match_rules` `exclude_from_home` `realtime_watch` |
...
| 7 | 其余 | 无圆点、无状态词（「空闲」不是需要看的状态） | — | 只显示 `最近扫描 X 前` 这行事实；实时监控关了才作为配置备注出现在库名下 |
...
**手机端**：同一行组件按断点切成卡片：第一行库名 + 「默认」标 + 可见范围胶囊 + ···，第二行小字
（类型 · 库存主数字 · 根目录 · 配置备注），之后是状态；状态格跨整行，进度条通栏填满卡片（桌面端受状态道宽度约束）。
「创建媒体库」在页头标题右侧。

**反馈**：动作成功/失败都走全站 toast（设默认、首页展示开关、删除、排序有一句回执），不用页顶
横条——列表长时用户在底部操作，看不到顶部。列表带 `role="list"/listitem` 语义（`aria-label="媒体库列表"`）。
