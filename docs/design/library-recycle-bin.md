...
> 三稿：「品质」独立成列，条目行、展开的每集都有；四稿：与「文件」合成一格两行——文件名在上、
> 解析出的品质在下（§2.5）。
>
...
┌──┬────────────────┬────────────────────────────────────────┬────────────────┬──────────┬────────────┐
│☐ │ 条目             │ 文件 · 品质                              │ 原因             │ 自动清理   │ 操作        │
│☐ │   九门 2025      │ 九门.2025.1080p.WEB-DL.H264.AAC-XXX.mkv  │ 洗版替换：…      │ 6 小时后   │ 恢复 清理    │
│  │   电影           │ 1080p WEB-DL · 4.2 GB · H.264 · AAC 2.0 · XXX │ 《九门》订阅洗版 │ 今天 22:40 │             │
│– │ › 权力的游戏     │ 24 集 · 3 季                              │ 洗版替换：…      │ 最早 9 小时后│ 恢复 24 清理 24│
│  │   剧集 · S01–S03 │ 720p HDTV · 26.4 GB · x264 · AC3 5.1 · CTU │ …                │ 最晚 3 天后│             │
│☑ │   S01E03 雪诺大人 │ Game.of.Thrones.S01E03.720p.HDTV.x264-CTU.mkv │                │ 9 小时后   │ 恢复 清理    │
│  │                  │ 720p HDTV · 1.1 GB · x264 · AC3 5.1        │                  │            │             │
│☐ │   S02E01 北境记住 │ Game.of.Thrones.S02E01.720p.HDTV.x264-KILLERS.mkv │ 720p → 1080p WEB │ 2 天后 │ 恢复 清理 │
│  │                  │ 720p HDTV · 0.9 GB · x264 · AAC 2.0 · KILLERS │                │            │             │
└──┴────────────────┴────────────────────────────────────────┴────────────────┴──────────┴────────────┘
...
- 条目行的「自动清理」取组内**最早**的 `purge_after`（"最早 9 小时后"，第二行"最晚 3 天后"），
  排序也按它；原因列组内一致时写整句，混合时写计数 `洗版证伪 4 · 洗版替换 2`；品质列组内
  一致时写一种，混合时写带计数的并列 `1080p WEB-DL 4 · 1080p BluRay 2`；
...
| **文件 · 品质** | 一格两行，是表格最宽的一列（约三分之一）。**第一行**：文件名（等宽，拿整格宽度，一般不再截断；悬停显示 `trash_original_path` 原路径，恢复回哪里一看就知道）；多文件条目写 `24 集 · 3 季` / `2 个版本`。**第二行**：从文件名解析出的品质，`分辨率 片源`（加粗，HDR 有值时追加金色徽标 `HDR10` / `DV`）`· 大小 · 视频编码[ 位深] · 音轨 · 制作组`，如 `1080p WEB-DL · 4.2 GB · H.264 · AAC 2.0 · XXX`。大小紧跟档位，是回收站最关心的数字。多文件条目第二行是组内汇总：档位一致写一种，混合写 `1080p WEB-DL 4 · 1080p BluRay 2`，大小为合计，编码/音轨/制作组去重后 `/` 并列。文件名本身就是发布名，下面一行是它的结构化读法，两行挨着比拆两列顺；与右侧「原因」列并排，洗版前后的档位对比一眼可见。「原地待回收」的 `原地` 警示胶囊（悬停说明"移入回收站失败，文件仍在原路径"）与批量清理失败的红字原因放在这一格最下面 | `file_name` `trash_original_path` `resolution` `media_source` `hdr` `size_bytes` `video_codec` `bit_depth` `audio_streams`（取首条音轨的 编码 + 声道）`release_group` |
...
展开后的文件行（缩进、略小一号）：☐ · `S01E03` + 集名（电影多版本时写版本规格）· **该集自己
的「文件 · 品质」两行**（文件名 / `720p HDTV · 1.1 GB · x264 · AC3 5.1`，制作组与条目行不同时才
写）· 自己的倒计时 · 恢复 / 清理。原因列不重复组内一致的整句，只在与组不同时写出（如某集实际
被 WEB-DL 而非 BluRay 替换）。
...
- 空状态：「回收站是空的」+ 一句解释：洗版替换下来的旧版本会在这里停留 7 天再自动删除，期间
  可以恢复。不放按钮。
- 手机端（`max-md`）：一个条目一卡：第一行海报 + 片名 + 最早到期倒计时（剧集在库名行追加
  `S01–S03 · 24 集`）；第二行文件名（多文件条目不写），第三行紧贴着它的品质一行，与桌面同一格
  两行的读法；第四行原因；底部左侧「展开 N 集」开关、右侧 `恢复 N` `清理 N`。展开后每集两行：
  集号 + 集名 + 倒计时，其下该集品质与大小，各带复选框。多选保留（复选框在卡片左侧），批量
  操作条改为底部悬浮。分页控件同桌面。
...
| `apps/web/lib/library-recycle.ts`（新）+ `test/library-recycle.test.mjs` | 纯函数：倒计时分档、摘要文案、字节格式化、组内原因合并、品质两行文案（单文件 / 组汇总）、半选态计算；`node --test` |
