...
┌──┬───────────────┬───────────────┬──────────────────────┬──────────────┬──────────┬────────────┐
│☐ │ 条目            │ 文件            │ 品质                   │ 原因           │ 自动清理   │ 操作        │
│☐ │   九门 2025     │ 九门.2025.108…  │ 1080p WEB-DL           │ 洗版替换：…    │ 6 小时后   │ 恢复 清理    │
│  │   电影          │                 │ 4.2 GB · H.264 · AAC…  │ 《九门》订阅洗版│ 今天 22:40 │             │
│– │ › 权力的游戏    │ 24 集           │ 720p HDTV              │ 洗版替换：…    │ 最早 9 小时后│ 恢复 24 清理 24│
│  │   剧集 · S01–S03│ 3 季            │ 26.4 GB · x264 · AC3…  │ …              │ 最晚 3 天后│             │
│☑ │   S01E03 雪诺大人│ Game.of.Thro…   │ 720p HDTV · 1.1 GB · … │                │ 9 小时后   │ 恢复 清理    │
│☐ │   S02E01 北境记住│ Game.of.Thro…   │ 720p HDTV · 0.9 GB · … │ 720p → 1080p WEB│ 2 天后    │ 恢复 清理    │
└──┴───────────────┴───────────────┴──────────────────────┴──────────────┴──────────┴────────────┘
...
| 文件 | 单文件：文件名（等宽）；多文件：`24 集` + 第二行 `3 季` / `2 个版本`。「原地待回收」形态加 `原地` 警示胶囊，悬停说明"移入回收站失败，文件仍在原路径"；批量清理失败的原因也标红在这里 | `file_name` + `trash_original_path IS NULL` |
| **品质** | **第一行（加粗）**：`分辨率 片源`（`1080p WEB-DL` / `2160p BluRay`），HDR 有值时追加金色徽标 `HDR10` / `DV`；**第二行（弱化）**：`大小 · 视频编码[ 位深] · 音轨 · 制作组`（`4.2 GB · H.264 · AAC 2.0 · XXX`）。大小放第二行开头，是回收站最关心的数字，列窄时也不能被省略号吃掉。多文件条目：第一行组内去重（一致写一种，混合写 `1080p WEB-DL 4 · 1080p BluRay 2`），第二行大小为组内合计、编码/音轨/制作组去重后 `/` 并列。与「原因」列并排，洗版前后的档位对比一眼可见 | `resolution` `media_source` `hdr` `size_bytes` `video_codec` `bit_depth` `audio_streams`（取首条音轨的 编码 + 声道）`release_group` |
...
**该集自己的品质两行**（`720p HDTV` / `1.1 GB · x264 · AC3 5.1`，制作组与条目行不同时才写）·
自己的倒计时 · 恢复 / 清理。原因列不重复组内一致的整句，只在与组不同时写出（如某集实际
...
- 手机端（`max-md`）：一个条目一卡：第一行海报 + 片名 + 最早到期倒计时（剧集在库名行追加
  `S01–S03 · 24 集`）；第二行就是**品质**两行，与桌面同一内容；文件名退到小字第三行（多文件
  条目不写）；第四行原因；底部左侧「展开 N 集」开关、右侧 `恢复 N` `清理 N`。展开后每集两行：
