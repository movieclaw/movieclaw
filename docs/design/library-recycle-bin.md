...
> 解析出的品质在下；展开的每集集号与集名也进第二行开头，文件格跨过「条目」列（§2.5）。
>
...
│☑ │      Game.of.Thrones.S01E03.720p.HDTV.x264-CTU.mkv          │                  │ 9 小时后   │ 恢复 清理    │
│  │      S01E03 雪诺大人 · 720p HDTV · 1.1 GB · x264 · AC3 5.1    │                  │            │             │
│☐ │      Game.of.Thrones.S02E01.720p.HDTV.x264-KILLERS.mkv      │ 720p → 1080p WEB │ 2 天后     │ 恢复 清理    │
│  │      S02E01 北境记住 · 720p HDTV · 0.9 GB · x264 · AAC 2.0 · KILLERS │          │            │             │
└──┴────────────────┴────────────────────────────────────────┴────────────────┴──────────┴────────────┘
...
都是从这个文件读出来的信息，放在一起；「条目」列不再重复剧名。其后是自己的倒计时、恢复 / 清理。原因列不重复组内一致的整句，只在与组不同时写出（如某集实际
...
- 空状态：「回收站是空的」+ 一句解释：洗版替换下来的旧版本会在这里停留 7 天再自动删除，期间
  可以恢复。不放按钮。
- 手机端（`max-md`）：一个条目一卡，读法与桌面同构——第一行海报 + 片名 + 最早到期倒计时（剧集
  在库名行追加 `S01–S03`）；第二行文件名（等宽，**最多折两行不截断**，手机上有纵向空间而文件名
  是要读的东西；多文件条目这行写 `24 集 · 3 季`）；第三行紧贴着它的品质一行（可换行）；第四行
  原因；底部左侧「展开 N 集」开关、右侧 `恢复 N` `清理 N`。展开后每集也是两行：文件名，其下
