...
> 解析出的品质在下；展开的每集集号与集名也进第二行开头，文件格跨过「条目」列；五稿：「自动清理」
> 倒计时并入操作格、放在按钮正上方（§2.5）。
>
...
都是从这个文件读出来的信息，放在一起；「条目」列不再重复剧名。其后是自己的「倒计时 / 恢复 清理」格。原因列不重复组内一致的整句，只在与组不同时写出（如某集实际
...
- 手机端（`max-md`）：一个条目一卡，读法与桌面同构——第一行海报 + 片名 + 库名（剧集追加
  ...
  换行）；第四行原因；底部左侧倒计时（24 小时内警示色）、右侧 `恢复 N` `清理 N`，与桌面同一对。展开后每集也是两行：文件名，其下
  `集号 集名 · 该集品质 · 大小`，右侧倒计时贯通两行，各带复选框。多选保留（复选框在卡片左侧），
  批量操作条改为底部悬浮。分页控件同桌面。
