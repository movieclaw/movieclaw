...
- 「待识别」从看不见的队列变成**条目上的状态**，但**与正式条目分组展示**
  （2026-09-04 实施期修订）：库页主墙只放正式条目（2:3 海报、拼音序、
  字母索引、分页），墙底单独一段「未识别 N」放临时条目（16:9 抓帧、
  按入账时间、不分页），段标题旁给「去待处理认领」入口。混排实测很糟——
  两种比例、两种身份塞进同一套拼音序，正片的墙被打散、认不出的文件又
  像正式条目。临时条目的**标题用条目目录名 / 文件名**（用户在文件管理器
  里看到的），不用解析器切出来的半截片名，也**不写推断年份**（mtime 推出
  的年份挂在认不出的片子上只会误导）。首页「最近添加」、站内搜索、Jellyfin
  「最新媒体」都只收正式条目（作品语义）；Jellyfin 的库内列表仍列出临时
  条目（可播）。接口口径：`/items?identity=confirmed|provisional`，其他库
  没有临时条目，`provisional` 恒空；
...
### 4.10 条目转移：搬的是文件，锚跟着改

放错库同样要有补救通道（`library/transfer.py`）。与影视库共用整条链路
（预览 → 确认 → 后台作业 → 进度/结论），但两处语义按能力位分叉，不按类型
字面判断：

- **搬运单元**：影视库条目 = 条目目录，整目录搬；本地内容库一文件一条目
  （4.1），条目就是那个文件，搬的是文件 + 它的附属文件（判定口径见
  `library.sidecar`：同名的 NFO、字幕等；台账里自成一条的邻居文件永远排除
  ——图片库里 `婚礼-cover.jpg` 完全可能是另一条独立条目），
  并保留它**相对库根的那段目录结构**——`2024旅行/` 是用户自己的分组目录，
  下面躺着几十条互不相干的条目。照抄影视库的"整目录搬"会把邻居一起卷走；
  把目标库里同名的 `2024旅行/` 判成冲突更是直接堵死转移，而那正是它该落
  进去的地方。判据是 `profile.scraped`（同 `local_external_id` 的分叉）；
- **身份锚随迁**：LOCAL 锚 = `{库 id}:path:{相对库根路径的摘要}`，搬完必须
  一并改写（`_relocate_item_identity`），否则源库里**后来**出现在同一相对
  路径的另一段录像算出来的锚与搬走那条一模一样，扫描会把它挂到已经搬去
  新库的条目上——一张卡片下面躺着两个库里两段不相干的视频。刮削归属
  （`scrape_library_id`）在它还指着源库时一并改挂（也是台账清空后的可见性
  兜底依据，见 library-access.md）。

TMDB 来源的条目不碰锚（锚是 TMDB id，与库和路径无关）；影视库里散在库根下
的临时本地身份也不碰——那种锚由解析证据派生，转移这里重算不出同一个值，
宁可留旧锚等重新识别改锚，也不写一个扫描对不上的新值。
