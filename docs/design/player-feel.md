...
- **direct/VOD 模式不防抖**：`seekBeyondBufferedRestarts === false` 时 seek
  就是播放器内跳转，成本近乎零，立刻执行手感最好。防抖只对「一次 seek =
  一次会话重启」的转码会话有意义。
...
`seekBeyondBufferedRestarts` 为 false 时，拖动中节流 100ms 实时 seek；为 true
的转码会话维持松手提交。风险：hls.js 在列表内跳转会取消在途分片请求，频繁
拖动反而更慢——所以节流 + 仅当 `video.seekable` 覆盖目标时才执行。
...
- **快退/快进步长可配置**：Jellyfin 默认后退 10 秒、前进 30 秒且可在设置里
  改；我们固定 ±10。做成播放设置里的两个选项。
- **起播 seek 收紧**：照 `setCurrentTimeIfNeeded`，目标与当前差 <1 秒就不 seek
  （省掉一次无谓的 seek 抖动）。
- **高码率下调小前向缓冲**：jellyfin-web 在 ≥25Mbps 且 Chrome/Edge/FF 时把
  `maxBufferLength` 从 30 压到 6，规避大分片吃不下（hls.js#876）。我们固定
  60 秒，先在 QoE 里看有没有对应现象，有再加这条件分支。
- **暂停时逐帧步进**（Jellyfin `seekFrames`，按流帧率算一帧的时长）。
