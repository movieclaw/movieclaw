...
### 5.1 NFO 从「读时解析」改为「入库吸收」（2026-09-12 用户决策）

上面第 1 层是**读时**读盘的：详情页每打开一次就回媒体盘解析一份条目 NFO，
分集区每打开一次把一季每集的 `.nfo` 都读一遍（40 集的季一次两百多个系统
调用）。而 NFO 的内容几乎从不变——它是刮削器或用户写完就放在那儿的。
把「一次性的事实」摊到每一次浏览上，且摊在 NAS 上最该少碰的那块盘，
是纯粹的浪费。用户拍板的口径：

> 入库把影片信息刮削到媒体库条目的时候，优先读 NFO；入库之后就只读已经
> 入库的信息；用户主动刷新元数据时会重读 NFO。

于是第 1 层不再是读路径的一层，而是**写路径的一步**：

```
入库/刷新：TMDB 档案落库 → 吸收条目目录的 NFO（有值的字段压过 TMDB）→ media_metadata
读取：     media_metadata / media_episode → TMDB 实时兜底
```

- 吸收点 `services/library/nfo_absorb`，由挂锚后的 `ensure_assets` 与单条目
  刷新 `_scrape` 调用——建档时文件台账行还没挂上条目，条目目录无从谈起，
  所以只能等到挂锚之后；
- **逐字段**合并而不是整份取舍：改动前 NFO 只要解出实质内容，详情页的
  简介/评分/类型/演职员就全以它为准、NFO 没写的字段显示为空。现在只写了
  `<plot>` 的 NFO 不会再把演员表清空；
- 演员按姓名与既有 TMDB 演员表对齐，把头像与影人 id 带过来（改动前读时
  `_fill_actor_thumbs` 做的事，挪到写时）；
- 分集的 `<aired>` **只在库里没有播出日期时**才采信：这一列不只是展示，
  订阅的期望集合与追新调度都按它判断已播/未播；
- 台账三列（2026-09-12 拆分，迁移 `c9f4a1e6b573`）：
  - `media_metadata.nfo_name`：出处标注的依据（详情页「信息来自 xxx.nfo」）；
  - `media_metadata.nfo_fingerprint`：「这个条目查过 NFO 没有」，NULL = 从未
    查过，是**存量回填的唯一判据**；找不到 NFO 时写空串；
  - `media_metadata.nfo_mirror_fingerprint` / `media_episode.nfo_mirror_fingerprint`：
    「磁盘上这份是不是**我们自己**写出去的」。**这才是跳过吸收的判据**——
    6.2 的媒体目录镜像每次刷新都按库内档案重写条目 NFO 与分集 NFO，认不出
    自家副本的话，下一次刷新读到的就是我们上一轮写出去的那份，它会把刚拉
    回来的 TMDB 新数据原样盖掉，正是 2026-08-04 决策要消灭的「NFO 挡住新
    数据」。

  **为什么必须拆成两列**：合一那版（`b7e2a9c4d318`）用「这份吸收过没有」当
  跳过判据，在两个方向上都错了。① 分集侧根本没有台账，于是集名与单集简介
  永久冻结在第一次镜像时的内容——新播剧集 TMDB 初期只有占位的「第 N 集」，
  几天后补上的真标题永远显示不出来，且**不需要用户有任何自定义 NFO 就会中**。
  ② 条目侧把「我们自己写的」和「用户的、没改过的」当成同一回事：刷新时
  `apply_display_profile` 刚把 TMDB 值无条件写回展示列，跳过吸收就等于丢掉
  用户的 NFO；关掉媒体目录镜像的库（磁盘上永远是用户那份、指纹永远不变）
  因此会发现自己的 NFO 在刷新后静默失效。拆开之后：自家副本跳过，用户的
  NFO **每次刷新都重压一遍**（解析有 mtime/大小缓存，重压不等于重读盘）。

  **镜像侧的对应守卫**：`nfo_fingerprint` 为 NULL（还没吸收过）的条目**不写
  条目 NFO**。档案里此刻没有用户 NFO 的任何字段，写出去就是拿纯 TMDB 内容
  盖掉用户用 TMM 刮的那份，而镜像会把指纹记进台账、回填从此再也挑不中它
  ——文件没了、库里也没有。守在镜像侧而不是补在调用方（选图 `set_artwork`
  就是这么漏的），任何未来的调用点都覆盖得到；图片不受此限；
- 存量条目（升级前入库、台账为 NULL）由后台任务 `backfill_nfo_absorption`
  补课：纯本地读盘、零 TMDB 请求，每 5 分钟一小批，补完自然空转。不补的话
  用 TMM 精心刮过的用户升级完会发现详情页换成了 TMDB 那份。
...
3. ~~**磁盘占用**：图片资产 GB 级下限可控；开口：设置页加「清理无引用资产」~~
   ...
   **2026-09-12 补第三条保留判据**：条目还挂着用户数据（观看状态 / 自建
   合集成员 / 分享链接）时也不删，资产跟着留。省下的那点磁盘换不来"删个库
   就把全家的观看进度 CASCADE 清空"——留下的条目不在任何库里，各处读路径
   都按库过滤，它是休眠的，同一部作品重新入库时靠身份锚自动接回。
