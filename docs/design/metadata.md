...
- 台账 `media_metadata.nfo_name` / `nfo_fingerprint`：前者是出处标注的依据
  （详情页「信息来自 xxx.nfo」），后者是「这份 NFO 的内容已经吸收过」。
  **指纹守卫是必需的**——6.2 的媒体目录镜像每次刷新都按库内档案重写条目
  NFO，镜像写完把新指纹记进台账；不比指纹的话，下一次刷新读到的就是我们
  自己上一轮写出去的那份，它会把刚拉回来的 TMDB 新数据原样盖掉，正是
  2026-08-04 决策要消灭的「NFO 挡住新数据」；
