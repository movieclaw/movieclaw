...
- **密度**收在页头右上角的「⋯」菜单里（三档单选，记住偏好），墙上不摆
  任何控件。
- **排序**同在「⋯」菜单里（`default` / `added_at` 两档，记住偏好，三面墙
  ——海报墙、相册墙、图廊——共用）。常驻序仍是拍摄时间倒序：一次导入几百张
  的入账时间挤在同一分钟里，按入账倒序等于目录遍历顺序；「最近添加」是持续
  往库里添内容的人自己选的一档。选了它之后同月的照片不再连续，月份分段与
  时间刻度都收起来，墙变成一条不分段的瀑布流（`PhotoWall grouped={false}`）。
- **虚拟化**：只挂视口上下各一屏半以内的瓦片，之外的连定位盒都不挂
  （`useTileWindow` / `lib/wall-window.ts`）。位置在渲染前已由 `layoutMasonry`
  算好、段高显式写在容器上，所以不挂瓦片既不影响滚动条也不影响滚动位置；
  最短列算法保证 `placements` 的 y 非降序，取窗口是两次二分。
  实测（5000 张、模拟 4× 降速的手机、414×896）：DOM 节点 35225 → 401，
  滑 40 屏后解码位图 366MB → 38MB，滚动 4.7fps → 59.7fps。
  开销与库大小基本无关：5 千 / 1 万 / 2 万 / 5 万张的滚动帧率都是 59+，
  解码位图恒为 36MB，主线程占用 0.32~0.37。两处专门为此做的事：
  月份分组按内容比对复用上一轮的数组（灯箱点收藏、追加一页只动一个月，
  不复用的话每段都要白重排一遍——五万张时 200ms）；远处的月份段按
  **距离预算**跳过量测（两百多段每帧各读一次布局要吃掉三成帧预算）。
  v1 曾用 `content-visibility:auto` 代替，但它只跳过绘制、节点与 `<img>`
  照样都在，且会让子树里的懒加载与解码完成通知失灵（见
  `components/poster-image.tsx`）——虚拟化之后瓦片不再需要它。
...
顺带修掉同一处的点击分区（用户反馈：看图要点两次）。触屏上原来是整张卡都
盖着一层「打开资源操作」的按钮，看图得先弹抽屉、再点抽屉里的「浏览图片」，
图片模式最主要的动作反而最远。改成**海报归看图、图外文字区归操作**：点海报
直接开灯箱（桌面触屏一致，也和媒体库一致），触屏点文字区才弹抽屉，桌面仍是
海报上的 hover 操作层。灯箱顶栏另塞「详情 / 投订阅 / 下载」——截图本来就是
「值不值得下」的判据，看完当场能下，不用退出灯箱再找按钮。

灯箱里点下载会弹「选择保存位置」，它压在灯箱（z-70）之上要置 `topmost`
（z-90），于是 `Modal` 顺手补了一条既有的层级漏洞：topmost 弹窗改在 capture
阶段监听 Esc 并掐断传播，一次 Esc 只关最上面那层——此前 topmost 的二次确认
弹窗被 Esc 关掉时，身下的弹窗也跟着一起关了。
