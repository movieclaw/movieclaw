...
> ✅ **已落地**：`GET /subscriptions/identity-audit`（管理员，纯只读，默认 30 天
> 窗口）。此前本节要的数字虽然都落了库，却**没有任何一处读它们**——
> `library_file.identity_doubt` 零读取点，投递活动 payload 的 `shadow` 键同样
> 零读取点，于是 §10.2 的观察期永远得不出结论（那条 113 MB 的假「正片」是
> 用户自己发现的，而系统连着两层都"看见"了）。端点给四组：
> `confidence`（本节点名的核心指标：只靠片名+年份认的投递占比）、
> `bitrate_shadow`（可疑档命中，决定要不要点灯）、
> `absurd_rejected`（极端档已生效，看有没有过度开火）、
> `runtime_doubt`（入库实测片长不符的文件）。每组带最近 20 条样本供人工核对。
