...
> 状态：**待实现**。起因是一例真实错配（§0）。行文沿用 `subscription.md` /
...
身份改挂时，除了给新条目做前向对账，还要给**被腾空的旧条目**做反向对账：
单元不再在库 → 工单退回 `WANTED` 并立即排队 → 订阅状态重算回 `active`。
...
新增 `subscription/wanted_fulfillment.py::reopen_unfulfilled_wanted(session, media_item_id)`：

- 取该条目 `status == IMPORTED` 的工单，与 `owned_units(media_item_id)` 求差集
...
- `claim.py::claim_files` —— 对已算好的 `displaced` 集合（97-101 行）逐个调用
- `claim.py::resolve_review` —— 同上，对 `displaced`
- `scan.py:4086` 重识别路径 —— 同上
...
`identity.py::match_identity` 的信号一（220-224 行）之后补一条：两边都有
`imdb_id`（或 `douban_id`）且**不相等** → `return None`。

现状是这种情况会掉到别名匹配继续走——站点已经明确说了"这是另一部片"，而我们
装作没看见。守卫方向与模块头"宁可漏，绝不静默错配"完全一致，零风险。
...
真正稳的是**相对比较**，而这恰好是 P5 的孪生判别场景：

> 同一个文件、同一个编码、同一个片源，"该用多少码率"这个未知量在两个孪生之间
> **被消掉了**。问题退化成"4GB 更像 88 分钟还是 210 分钟"——答案毫无悬念。

所以 P2 的形态是 **P5 的判别器**，而不是独立门禁：歧义条目遇到 `title_year`
候选时，算每个孪生的隐含码率，取最接近该「分辨率+片源+编码」典型区间的那个。
...
| 详情页 ID 与条目不等 | 一票否决 + 拉黑，记 `MATCH_REJECTED`（写明双方 ID） |
...
最后一格的实际频率：P3 与 P2 会把绝大多数情况自动裁决掉，剩下的只有"两部同名
同年片、站点都没标 IMDb、时长还差不多"——那才是真·极端特例，到那个份上确实该
问人。**这是 P5 值得做的核心理由**：交互成本不是"每次都问"，而是"几乎不问，
问的时候一定值得问"。

交互复用告警中心，不需要新页面：两个按钮「就是这部，下载」（投递 + 白名单）
/「不是，别再推荐」（拉黑，形状照抄 `matching.py:140` 的 `upgrade_excluded`），
同时记一条 `ActivityType.MATCH_PENDING_CONFIRM`。

歧义订阅还要改一处调度语义：P3 的详情页复核对普通订阅是"能拿就拿，拿不到放行"，
对歧义订阅必须是"**拿不到就不投**"。
...
- **不做孪生别名差集**（"只用本条目独有的别名匹配"）：§0 的 case 里
  ...
  不能靠削弱别名。
- **TMDB 不可达**：当次不收紧、不缓存，等下次。绝不能让网络抖动把订阅卡成待确认。
- 别忘了另外三个 `MediaIdentity` 构造点也要带上歧义标记：`manual_grab.py:184`、
  `replacement.py:481`、`release_forecast.py:202`——判定口径不一致是这套机制最
  容易出的裂缝（`load_season_titles` 的注释里已经踩过一次同类的坑）。

### 9.6 验收标准

```
1. 条目《奥德赛》(2026)、TMDB 存在孪生《The Odyssey》(2026)
   → 探测后 identity_twins 有一条，且缓存命中后不重复请求 TMDB
2. 歧义条目 + title_year 候选 + 详情页 ID 等于孪生 → 断言否决且不产生 SystemNotice
3. 歧义条目 + title_year 候选 + 无任何 ID + P2 分不出 → 断言不投递、点亮待确认告警
4. 用户点「就是这部」→ 断言投递发生；点「不是」→ 断言拉黑且后续不再选中
5. TMDB 探测抛异常 → 断言按非歧义处理（照常投递）
```
