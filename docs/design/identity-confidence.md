...
P3/P4 落地后跑一次一次性审计任务，把**已经躺在用户库里**的疑似错配找出来。

### 11.2 理由

这个洞不是今天才有的，§0 这一例几乎不可能是唯一一例。P1 之后新入库的文件才
有 `identity_source` 分级，存量文件全是 NULL——不主动回头看，历史错配会一直
静静躺在库里，直到用户某天点开发现片不对。

**这是本方案里唯一一项"交付价值"而非"防止未来出错"的工作**，对用户的体感
最直接。

### 11.3 做法

对 `library_file` 里 `source == IMPORTED`、带 `site_id`/`torrent_id` 来源戳、
且 `kind == movie` 的行：

1. 跑一遍 §8.3 的时长比对（`duration_seconds` 台账已有，不用重新 ffprobe）
2. 命中的再跑一遍 §7 的详情页 ID 复核（数量少，一次性任务，不怕慢）
3. 结果汇总成**一条** SystemNotice（不是每个文件一条——存量可能几十个，
   逐条点灯是灾难），点开是一张清单，每行给"去修正识别结果"入口

挂在 `movieclaw_scheduler` 上作为可手动触发的维护任务（不是定时任务：跑一次
就够了，日常由 P4 在入库时接管）。

### 11.4 验收标准

```
1. 库里植入一个时长严重不符的存量文件 → 断言审计能捞出来
2. 库里有 20 个可疑文件 → 断言只点亮 1 条 SystemNotice，清单含 20 行
3. 审计任务重复跑两次 → 断言不产生重复告警（dedupe_key 幂等）
4. 断言审计只读、不修改任何身份（发现 ≠ 自动翻案，拍板永远归用户）
```
