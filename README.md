...
界面是液态玻璃：侧栏、输入框、悬浮按钮都在实时折射你设的那张背景图，边缘带一点色差。
...
手机上是同一套界面，不是另做一个阉割版。iOS 里「添加到主屏幕」按独立 App 跑，
...
- **手机上是同一套界面。** iOS 里「添加到主屏幕」就是一个独立 App，没有地址栏，
  刘海和底部指示条都让开了。界面是液态玻璃，面板实时折射你自己设的背景图。
